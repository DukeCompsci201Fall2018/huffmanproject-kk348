...
//	    }
	...
	private void writeCompressedBits(String[] codings, BitInputStream in, BitOutputStream out) {
		...
		String finalcode = codings[PSEUDO_EOF];
		out.writeBits(finalcode.length(), Integer.parseInt(finalcode,2));
	...
	private HuffNode makeTreeFromCounts(int[] counts) {
		...
		for (int i = 0; i < counts.length; i++) {
			if (counts[i] > 0) {
			    ...
			    System.out.printf("pq with %d nodes\n", pq.size());
			}
		}
	...
	public void decompress(BitInputStream in, BitOutputStream out){
		...
		//System.out.println("root: " + root.myValue + " " + root.myLeft.myLeft.myLeft.myLeft.myValue );
	...
	private HuffNode readTreeHeader(BitInputStream in) {
		...
		if (bit == -1) {
			throw new HuffException("not found");
		}
		
		if (bit == 0) {
			HuffNode left = readTreeHeader(in); 
			HuffNode right = readTreeHeader(in);
			//System.out.println(left.myValue + " " +  right.myValue);
